...
import {
    User, Calendar, Package, LogOut, Clock, Heart, Edit2, Trash2,
...
export default function ProfilePage() {
    ...
    const loadWishlist = async () => {
        ...
        try {
            ...
            if (ids.length === 0) { setWishlistPackages([]); return; }
    ...
    const isAdmin = (user as { role?: string }).role === 'admin' || user.email === 'admin@ceylonroots.com';
                ...
                <div className="flex-1 p-4">
                    <div className="flex items-start justify-between gap-2 mb-2">
                        <div>
                            ...
                            <p className="text-xs text-gray-400 font-mono mt-0.5">#{booking.id.slice(-8).toUpperCase()}</p>
                        </div>
                        <span className={`text-xs px-2.5 py-1 rounded-full border font-medium capitalize shrink-0 ${statusColors[booking.status] ?? 'bg-gray-100 text-gray-700'}`}>
                            {booking.status}
                        </span>
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                        <span className="font-bold text-ceylon-tea">${booking.totalAmount.toLocaleString()}</span>
    ...
    const EmptyBookings = ({ label }: { label: string }) => (
        <div className="text-center py-12 text-gray-400">
            ...
            <Link href="/packages">
                <Button variant="outline" size="sm" className="mt-4 text-xs">Browse Packages</Button>
            </Link>
        </div>
    );
            ...
            <div className="ceylon-container py-8">
                <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
                    ...
                    <div className="lg:col-span-1 space-y-4">
                        <Card>
                            <CardContent className="pt-6 pb-5 px-5">
                                <div className="flex flex-col items-center text-center mb-5">
                                    <div className="w-20 h-20 rounded-full flex items-center justify-center text-2xl font-bold text-white mb-3 shadow-md overflow-hidden"
                                        style={{ background: 'linear-gradient(135deg, #2E8B57, #1A5276)' }}>
                                        {user.image
                                ...
                                <div className="grid grid-cols-2 gap-2 text-center text-xs mb-5">
                                    <div className="bg-gray-50 rounded-lg py-2">
                                        <div className="font-bold text-xl text-ceylon-tea">{bookings.length}</div>
                                        <div className="text-gray-400">Bookings</div>
                                    </div>
                                    <div className="bg-gray-50 rounded-lg py-2">
                                        <div className="font-bold text-xl text-ceylon-ocean">{upcoming.length}</div>
                        ...
                        <Card>
                            <CardContent className="py-3 px-5">
                                <p className="text-xs font-medium text-gray-400 uppercase tracking-wider mb-2">Quick Links</p>
                                <div className="space-y-1">
                                    {[
                                        { href: '/packages', label: 'Browse Packages', icon: <Package className="h-3.5 w-3.5" /> },
                                        { href: '/wishlist', label: 'My Wishlist', icon: <Heart className="h-3.5 w-3.5" /> },
                                        { href: '/guides', label: 'Find a Guide', icon: <User className="h-3.5 w-3.5" /> },
                                    ].map(link => (
                    ...
                    <div className="lg:col-span-3">
                        <Tabs defaultValue="bookings" onValueChange={(v) => { if (v === 'wishlist') loadWishlist(); }}>
                            <TabsList className="grid grid-cols-3 mb-5 bg-white border border-gray-100 rounded-xl p-1 shadow-sm">
                                <TabsTrigger value="bookings" className="rounded-lg text-sm data-[state=active]:bg-ceylon-tea/10 data-[state=active]:text-ceylon-tea">
                                    <Package className="h-3.5 w-3.5 mr-1.5" /> Bookings
                                </TabsTrigger>
                                <TabsTrigger value="wishlist" className="rounded-lg text-sm data-[state=active]:bg-ceylon-tea/10 data-[state=active]:text-ceylon-tea">
                                    <Heart className="h-3.5 w-3.5 mr-1.5" /> Wishlist
                                </TabsTrigger>
                                <TabsTrigger value="settings" className="rounded-lg text-sm data-[state=active]:bg-ceylon-tea/10 data-[state=active]:text-ceylon-tea">
                                        ...
                                        {bookingsLoading ? (
                                            ...
                                            <Tabs defaultValue="all">
                                                ...
                                                {[
                                                    ...
                                                    { key: 'cancelled', list: cancelled, label: 'cancelled' },
                                                ].map(({ key, list, label }) => (
                                                    <TabsContent key={key} value={key}>
                                                        {list.length === 0
                                                            ? <EmptyBookings label={label} />
                                                            : <div className="space-y-3">{list.map(b => <BookingCard key={b.id} booking={b} />)}</div>
                                                        }
                                                    </TabsContent>
                                                ))}
                                            </Tabs>
                                    ...
                                    <CardContent className="pt-5">
                                        {wishlistLoading ? (
                                            ...
                                            <div className="text-center py-12">
                                                ...
                                                <p className="text-gray-500 text-sm mb-2">Your wishlist is empty</p>
                                                <p className="text-gray-400 text-xs mb-5">
                                                    Click the heart icon on any package to save it here.
                                                </p>
                                                <Link href="/packages">
                                                    <Button size="sm" className="bg-ceylon-tea hover:bg-ceylon-tea/90 text-white">
                                                        Browse Packages
                                                    </Button>
                                                </Link>
                                            </div>
                                            ...
                                            <div className="space-y-3">
                                                {wishlistPackages.map(pkg => (
                                                    <div key={pkg.id} className="flex items-center gap-4 p-3 border border-gray-100 rounded-xl hover:bg-gray-50 transition-colors">
                                                        <div className="w-20 h-16 rounded-lg overflow-hidden flex-shrink-0">
                                                            <img
                                                                src={pkg.imageUrl ?? pkg.image ?? '/placeholder.jpg'}
                                                                alt={pkg.title}
                                                                className="w-full h-full object-cover"
                                                            />
                                                        </div>
                                                        <div className="flex-1 min-w-0">
                                                            <h4 className="font-semibold text-gray-800 text-sm truncate">{pkg.title}</h4>
                                                            <div className="flex items-center gap-3 text-xs text-gray-400 mt-1">
                                                                <span className="flex items-center gap-1">
                                                                    <Clock className="h-3 w-3" /> {(pkg as { duration?: number; durationDays?: number }).duration || (pkg as { duration?: number; durationDays?: number }).durationDays} days
                                                                </span>
                                                                <span className="flex items-center gap-1">
                                                            ...
                                                            <p className="font-semibold text-ceylon-tea text-sm mt-1">
                                                                {format(typeof pkg.price === 'object' ? Number(pkg.price) : pkg.price)}
                                                            </p>
                                                        </div>
                                    ...
                                    <Card>
                                        <CardContent className="pt-5">
                                            <h3 className="font-semibold text-gray-800 mb-4 flex items-center gap-2">
                                                <Edit2 className="h-4 w-4 text-ceylon-tea" /> Profile Information
                                            </h3>
                                            {editing ? (
                                                <div className="space-y-4">
                                                    <div>
                                        ...
                                        <CardContent className="pt-5">
                                            ...
                                            {!confirmDelete ? (
                                                ...
                                                <div className="bg-red-50 border border-red-200 rounded-xl p-4">
                                                    <p className="text-sm font-semibold text-red-700 mb-2">Are you absolutely sure?</p>
                                                    <p className="text-xs text-red-500 mb-4">
                                                        This will permanently delete your account, all bookings, and saved data.
                                                    </p>
                                                    <div className="flex gap-2">
                                                        <Button
                                                            size="sm"
