...
const ComponentSelector: React.FC<ComponentSelectorProps> = ({
    ...
    <div className="border border-ceylon-sand/30 rounded-lg overflow-hidden transition-all hover:shadow-md group mb-4">
      ...
      <div className="p-4">
        <div className="flex justify-between items-start mb-2">
          <h3 className="font-bold text-base text-ceylon-stone line-clamp-1">
            {component.name}
          </h3>
        ...
        {component.type === "transport" && (
          <div className="mb-3">
            ...
            <div className="text-sm text-gray-600">
              <span>
                From: {(component as TransportComponent).departureLocation}
              </span>
            </div>
            <div className="text-sm text-gray-600">
              <span>
                To: {(component as TransportComponent).arrivalLocation}
              </span>
            </div>
          </div>
        )}
        ...
        {component.type === "destination" && (
          <div className="mb-3">
            ...
            <div className="flex flex-wrap gap-1">
              {(component as DestinationComponent).attractions
                .slice(0, 3)
                .map((attraction, index) => (
                  <span
                    key={index}
                    className="bg-ceylon-sand/20 text-ceylon-stone text-xs px-2 py-0.5 rounded-full"
                  >
                    {attraction}
                  </span>
                ))}
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm p-5 border border-ceylon-sand/30">
      <h2 className="text-xl font-bold text-ceylon-stone mb-5">
        Travel Components
      </h2>
      ...
      <Tabs defaultValue="all">
        <TabsList className="w-full grid grid-cols-5 mb-5 bg-ceylon-sand/10 p-1 rounded-lg">
          <TabsTrigger value="all" className="py-2 rounded-md">
            All
          </TabsTrigger>
          <TabsTrigger
        ...
        {isLoading ? (
          ...
          <div className="h-[500px] flex flex-col items-center justify-center bg-ceylon-sand/5 rounded-xl p-6 text-center">
            ...
            <h3 className="text-lg font-bold text-ceylon-stone mb-2">
          ...
          <>
            ...
            >
              {filterComponents(
                components.filter((c) => c.type === "accommodations")
              ).map((component) => (
                <ComponentCard key={component.id} component={component} />
              ))}
              {filterComponents(
            ...
            >
              {filterComponents(
                components.filter((c) => c.type === "destination")
              ).map((component) => (
                <ComponentCard key={component.id} component={component} />
              ))}
              {filterComponents(
            ...
            >
              {filterComponents(
                components.filter((c) => c.type === "activities")
              ).map((component) => (
                <ComponentCard key={component.id} component={component} />
              ))}
              {filterComponents(
            ...
            >
              {filterComponents(
                components.filter((c) => c.type === "transport")
              ).map((component) => (
                <ComponentCard key={component.id} component={component} />
              ))}
              {filterComponents(
