...
const DestinationCard: React.FC<DestinationCardProps> = ({ destination }) => {
    ...
    return (
        <div className="flex flex-col bg-card rounded-2xl shadow-lg overflow-hidden hover:shadow-2xl transition-shadow duration-300 border border-border">
            {/* Image Section */}
            <div className="relative w-full h-56 sm:h-64">
                ...
                />
                <span className="absolute top-4 left-4 bg-ceylon-tea text-white px-3 py-1 rounded-full text-xs font-semibold shadow">
            ...
            <div className="flex flex-col flex-1 p-6">
                ...
                {destination.description ? (
                    ...
                    <p className="text-muted-foreground/60 italic mb-3">No description available</p>
                )}
                    ...
                    {destination.attractions?.length > 0 ? (
                        <div className="flex flex-wrap gap-2 mt-1">
                            {destination.attractions.slice(0, 3).map((attraction, index) => (
                                <span
                                    key={index}
                                    className="bg-ceylon-tea/10 text-ceylon-tea px-2 py-1 rounded-full text-xs"
                                >
                                    {attraction}
                                </span>
                            ))}
                        ...
                        <p className="text-muted-foreground/60 italic text-sm mt-1">No attractions listed</p>
                    )}
                </div>
