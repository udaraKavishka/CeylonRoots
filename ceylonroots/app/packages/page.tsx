      ...
      try {
        ...
        const mappedPackages = data.map((pkg: BackendPackage) => {
          ...
          return {
            ...
            itineraryDays: (pkg.itineraryDays || []).map(
              (day: {
                dayNumber: number;
                title: string;
                description: string;
                ...
                activities: day.activities
                  ? Array.isArray(day.activities)
                    ? (day.activities as ({ name?: string } | string)[]).map(
                        (act) =>
                          typeof act === "string"
                            ? { name: act }
                            : { name: act.name || "" }
    ...
    if (selectedDuration !== "Any Duration") {
      if (selectedDuration === "15+ Days") {
        ...
        result = result.filter(
          (pkg) => pkg.duration >= min && pkg.duration <= (max || min)
        );
      }
    }
        ...
        <div className="bg-gradient-to-r from-ceylon-teal-700 to-ceylon-teal-900 py-16 px-4">
          ...
          >
            ...
            <p className="text-black text-xl max-w-3xl">
              Explore our carefully curated travel packages designed to showcase
              the best of Sri Lanka&apos;s natural beauty, rich history, and
              vibrant culture.
            </p>
          </motion.div>
        </div>
