...
const CostEstimator: React.FC<CostEstimatorProps> = ({
  ...
  const typeGroups = components.reduce(
    (groups, item) => {
  ...
  return (
    <div className="bg-card rounded-xl shadow-sm p-5 border border-ceylon-sand/30">
      <h2 className="text-xl font-bold text-ceylon-stone mb-4 flex items-center">
        <span className="bg-ceylon-tea text-white rounded-full w-6 h-6 flex items-center justify-center mr-2">
          3
        </span>
        Cost Breakdown
      </h2>
      ...
      <div className="space-y-4">
        {Object.entries(typeGroups).map(([type, items]) => (
          <div
            key={type}
            className="border-b border-ceylon-sand/20 pb-4 last:border-0 last:pb-0"
          >
      ...
      <div className="mt-6 pt-4 border-t border-ceylon-sand/30 flex justify-between items-center">
        ...
        <span className="text-2xl font-bold text-ceylon-tea">
          Rs{totalCost}
        </span>
      </div>
    </div>
