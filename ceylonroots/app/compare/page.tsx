...
import { ArrowLeft, Star, Calendar, MapPin, CheckCircle, XCircle, GitCompare } from 'lucide-react';
import {
    RadarChart,
    PolarGrid,
    PolarAngleAxis,
...
export default function ComparePage() {
    ...
    if (compareList.length === 0) {
        return (
            <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center text-center px-4">
                ...
                <h1 className="text-2xl font-bold text-gray-700 mb-2">No packages to compare</h1>
                <p className="text-gray-500 mb-6">
                    Select up to 3 packages from the packages page to compare them side-by-side.
                </p>
                <Button asChild className="bg-ceylon-tea hover:bg-ceylon-tea/90 text-white">
    ...
    const radarData = [
        {
            subject: 'Rating',
            ...Object.fromEntries(compareList.map(p => [p.title, parseFloat(p.rating.toFixed(1))])),
            fullMark: 5,
        },
        {
            subject: 'Value',
            ...Object.fromEntries(compareList.map(p => [
                p.title,
        ...
        },
        {
            ...
            ...Object.fromEntries(compareList.map(p => [
                p.title,
                parseFloat((maxReviews > 0 ? (p.reviewCount / maxReviews) * 5 : 0).toFixed(1)),
            ])),
            fullMark: 5,
        },
        {
    ...
    const rows: { label: string; render: (pkg: typeof compareList[0]) => React.ReactNode }[] = [
        {
            label: 'Price',
            render: pkg => (
                <span className="text-lg font-bold text-ceylon-tea">{format(pkg.price)}</span>
            ),
        },
        {
            label: 'Duration',
        ...
        },
        {
            ...
            render: pkg => (
                <span className="flex flex-col items-center gap-1">
                    {(pkg.regions ?? []).map(r => (
                        <span key={r} className="inline-flex items-center gap-1 text-sm text-gray-600">
            ...
            render: pkg => (
                <div className="flex flex-wrap justify-center gap-1">
                    {(pkg.themes ?? []).slice(0, 4).map(t => (
                        <span key={t} className="px-2 py-0.5 bg-ceylon-tea/10 text-ceylon-tea text-xs rounded-full">
                            {t}
                        </span>
                    ))}
                </div>
        ...
        },
        {
            ...
            render: pkg => (
                <ul className="text-sm text-gray-600 text-left space-y-1 max-w-[180px]">
                    {((pkg.priceIncludes ?? pkg.includes) ?? []).slice(0, 4).map((item, i) => (
                        <li key={i} className="flex items-start gap-1">
                            <CheckCircle className="h-3.5 w-3.5 text-green-500 mt-0.5 flex-shrink-0" />
                            {item}
                        </li>
        ...
        },
        {
            ...
            render: pkg => (
                <ul className="text-sm text-gray-600 text-left space-y-1 max-w-[180px]">
                    {((pkg.priceExcludes ?? pkg.excludes) ?? []).slice(0, 4).map((item, i) => (
                        <li key={i} className="flex items-start gap-1">
            ...
            <div className="max-w-7xl mx-auto px-4 py-8 space-y-10">
                            ...
                            <tbody>
                                {rows.map((row, idx) => (
                                    ...
                                    >
                                        ...
                                        {compareList.map(pkg => (
                                            <td key={pkg.id} className="py-4 px-6 text-center align-top">
                                                {row.render(pkg)}
                                            </td>
                                        ))}
                                    </tr>
                ...
                {compareList.length >= 2 && (
                    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
                        <h2 className="text-xl font-bold text-gray-800 mb-1">Visual Comparison</h2>
                ...
                <div className={`grid gap-4 ${compareList.length === 1 ? 'grid-cols-1 max-w-sm mx-auto' : compareList.length === 2 ? 'grid-cols-2' : 'grid-cols-3'}`}>
                    {compareList.map(pkg => (
                        <div key={pkg.id} className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 flex flex-col gap-2">
