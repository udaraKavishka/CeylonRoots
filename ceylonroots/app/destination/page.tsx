...
const Destinations = () => {
    ...
    const [filteredDestinations, setFilteredDestinations] =
        useState<DestinationDetails[]>(allDestinations);
    ...
    useEffect(() => {
        if (activeRegion && activeRegion !== "all") {
            setFilteredDestinations(
                allDestinations.filter((dest) => dest.region === activeRegion)
            );
        } else {
    ...
    const regions = [
        "all",
        ...Array.from(new Set(allDestinations.map((dest) => dest.region))),
    ];
                ...
                <div className="bg-gradient-to-r from-ceylon-tea/20 to-ceylon-spice/10 py-16">
                    <div className="ceylon-container">
                        <h1 className="text-4xl md:text-5xl font-bold mb-6">
                            ...
                            <span className="text-ceylon-spice">Treasures</span>
                        </h1>
                            ...
                            {regions.map((region) => (
                                <Button
                                    ...
                                    className={
                                        activeRegion === region ||
                                            (region === "all" && !activeRegion)
                                            ? "bg-ceylon-tea hover:bg-ceylon-tea/90"
                                            : ""
                                    }
                                >
