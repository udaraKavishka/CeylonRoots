...
const CustomizePage = () => {
  ...
  useEffect(() => {
    const newTotalCost = selectedComponents.reduce(
      (sum, item) => sum + item.price,
      0
    );
    setTotalCost(newTotalCost);

    const accommodations = selectedComponents.filter(
      (item) => item.type === "accommodations"
    ).length;
  ...
  const handleSubmitQuotation = async (email: string) => {
    ...
    try {
      ...
      toast({
        ...
        description:
          "Our team will review your custom package and send a quote to your email soon.",
        className: "bg-ceylon-tea text-white border-ceylon-tea-dark",
      });
    } catch {
      toast({
        ...
        description:
          "Could not submit your quotation request. Please try again.",
        variant: "destructive",
      });
    } finally {
              ...
              <div className="grid grid-cols-1 gap-6">
                <div className="bg-white rounded-xl shadow-lg p-5 border border-ceylon-tea/20">
                  <div className="flex items-center mb-4">
                    ...
                    <h2 className="text-xl font-bold text-ceylon-stone">
                  ...
                  {selectedComponents.length === 0 && (
                    <div className="text-center py-10 text-gray-600 bg-ceylon-sand/10 rounded-lg border border-dashed border-ceylon-tea/30">
                      ...
                      <p className="text-sm mt-1">
                        Add components from the selector to build your travel
                        plan
                      </p>
                    </div>
                  )}
